	...
	if err := wait.PollImmediate(interval, timeout, func() (bool, error) {
		...
		for _, item := range l.Items {
			...
			if item.GetCondition(v1.TypeHealthy).Status != corev1.ConditionTrue {
				t.Logf("The status of installed package: %v is %v and the condition of Package Revision is %v", item.Name, item.Status, item.Status.ConditionedStatus)
				return false, nil
			}
		}
		return true, nil
...
func WaitForAllPackagesDeleted(ctx context.Context, c client.Client, interval time.Duration, timeout time.Duration, t *testing.T) error {
	return wait.PollImmediate(interval, timeout, func() (bool, error) {
		...
		for _, item := range l.Items {
			t.Logf("Name: %v \t Type: %v from Registry: %v is left undeleted", item.Name, item.Kind, item.ObjectMeta.Name)
		}
